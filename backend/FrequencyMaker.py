...
class FrequencyMaker:
    ...
    def chord_stacker(self, chord_root):
        stack = []
        # octave below
        print(type(self.chromatic[0]), self.chromatic)
        print(type(self.scale), self.scale)
        print(type(chord_root), chord_root)
        octave_below = self.chromatic[self.scale[chord_root]] / 2.0
        ...
        for x in range(3):
            ...
            scale_index = chord_root + (x * 2)

            # if the scale_index is out of range of self.scale, mod scale_index to within the range, then multiply
            # given freq by 2 to send it up an octave
        ...
        # octave above
        octave_above = self.chromatic[self.scale[chord_root]] * 2
